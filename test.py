...
start_point = None
end_point = None
xlsx_file_path = "C:\\Users\\DELL\\PycharmProjects\\pythonProjecttest\\measurements.xlsx"
results = []
...
selected_option = st.sidebar.radio("Select", ["Home", "Login", "Logout", "Register"])
...
if selected_option == "Login":
    ...
    if st.button("Login"):
        if username == "Habiba" and password == "Ihf07121999":
...
selected_option = st.selectbox("Select", ["Upload Images", "User", "Report"])

# Create buttons for navigation based on the selected option
#if selected_option == "Home":
    #st.write("Welcome to the Home Page.")
if selected_option == "Upload Images":
    ...
    if uploaded_first_image is not None and uploaded_second_image is not None:
        st.image(uploaded_first_image, caption="First Image", use_column_width=True)
        st.image(uploaded_second_image, caption="Second Image", use_column_width=True)
    ...
    report_df = pd.read_excel(xlsx_file_path)

    if not report_df.empty:
        st.dataframe(report_df)
        if "Result" in report_df.columns and "Distance (mm)" in report_df.columns:
            st.subheader("Line Chart")
            fig, ax = plt.subplots(figsize=(8, 6))

            x = report_df["Distance (mm)"]
            y = report_df["Result"]
            #y = report_df["Distance (mm)"]

            ax.plot(x, y, marker='o', color='skyblue', linestyle='-')
            ax.set_xlabel("Distance (mm)")
            ax.set_ylabel("Result")
            ax.set_title("Line Chart of Distance Measurements")
            st.pyplot(fig)
#else:
